...
- **Phase 5** 11. `rg -n -F 'processActivityImageMessage' bn88-backend-v12\src\services\activity\processActivityImageMessage.js -S` 12. `rg -n -F 'ChatSession' bn88-backend-v12\src\lib\prisma.ts -S`
...
- ManyChat flow engine entirely unimplemented.
...
ได้ครับพี่ ✅ **เพิ่มเข้าไปได้เลย** และผมจัดให้เป็น “Section ต่อท้าย” ที่ **สไตล์/โครงเหมือนเอกสาร Master Work Plan เดิม** (Verify / Implement / Acceptance criteria) โดย **ไม่รื้อ Phase เดิม** — แค่ “เติมเพิ่มท้ายไฟล์” เท่านั้น

ด้านล่างคือ **ข้อความที่พี่ copy ไปแปะ “ต่อท้ายบรรทัดสุดท้าย”** ของเอกสารเดิมได้ทันที 👇

---

## 10. ManyChat-style Flow Engine (Quick Reply Sessions + Follow-up/Retry + Multi-channel Adapters)

This section adds a ManyChat-like flow engine that supports Quick Replies with **Follow-up**, **Retry**, and **Session state**, and works across **LINE / Telegram / FB Messenger / Webchat** using a single core engine + per-channel payload adapters. This is implemented as an additive module (minimal diff), without changing existing public APIs.

---

ได้เลยพี่ ✅ ด้านล่างนี้คือ **ข้อความ “ต่อท้าย”** (Append) ที่พี่เอาไปแปะ **ต่อท้ายบรรทัดสุดท้าย** ของ Master Work Plan เดิมได้ทันที — สไตล์เดียวกัน (Architecture/Phase/Checks/Acceptance) และ **ไม่รื้อ Phase เดิม** แค่ “เพิ่มท้ายไฟล์”
...
“ต่อเข้าของจริง” ต่อจากนี้ แบบไม่เดาเลย: พี่เปิด ngrok แล้วเอา **request body ของ LINE postback 1 อัน** จาก `http://127.0.0.1:4040` มาแปะ เดี๋ยวผมชี้ “ต้อง hook ที่ route ไหน” และ “parse postback data ตรงไหน” ให้จบเป็นจุดๆ
